...
| **LfeCut** | 40 / 60 / 80 / 100 / 120 Hz sub-bass crossover | 80 Hz |
| **LfeOverlap** | None / 20 / 40 Hz band shared by the sub and the mains | 20 Hz |

**LfeCut** sets the corner below which content is handed to the LFE bed: the fronts (stereo) or every channel (5.1/7.1) have that content summed to the sub. **LfeOverlap** sets how far *below* that corner the speakers keep playing. At None the two filters are complementary and every frequency is reproduced exactly once - set LfeCut to your front speakers' own low-frequency limit and the sub takes over precisely where they stop. Any other value leaves a band that the sub and the mains both carry, summing to a broad lift of roughly +2-3 dB through the low bass; the default 80 Hz / 20 Hz pair sends everything under 80 Hz to the sub while the fronts still run down to 60 Hz. Worth an ear: the two sources arrive from different points in the room, so how much the shared band actually gains depends on your sub's placement and phase. **Mode** controls input detection. Auto analyses channel activity and selects the appropriate path. **Speakers** is used by the channel-based fallback when spatial audio is unavailable. **Volume** applies a master gain to everything the plugin emits (spatial objects and channel fallback alike), useful when shared-mode Atmos arrives quieter than stereo. Note it only affects audio that flows through the Windows shared-mode mixer — exclusive-mode and bitstream-passthrough streams bypass all APOs and are untouched.
